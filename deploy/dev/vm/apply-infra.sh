...
ATMS_SOX_DB_PASSWORD=${ATMS_SOX_DB_PASSWORD}
...
docker exec -i \
  ...
  -e ATMS_SOX_DB_PASSWORD="${ATMS_SOX_DB_PASSWORD}" \
...
if ! wait_for "Kafka readiness" "docker inspect --format '{{if .State.Health}}{{.State.Health.Status}}{{else}}none{{end}}' sitionix-kafka | grep -qx healthy" 36 5 5; then
  dump_kafka_diagnostics
  exit 1
fi
